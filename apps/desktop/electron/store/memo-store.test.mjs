...
test("memo store replace inserts a memo with the given id and timestamps", async () => {
  await withTempStore(async (store) => {
...
test("memo store replace overwrites an existing memo with the same id", async () => {
  await withTempStore(async (store) => {
